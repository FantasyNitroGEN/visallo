...
public class ImportJsonMRMapper extends VisalloElementMapperBase<SequenceFileKey, Text> {
    ...
    private void mapProductionCompany(int productionCompanyId, JSONObject json, Context context) {
        ...
        VisalloProperties.CONCEPT_TYPE.setProperty(productionCompanyMutation, TheMovieDbOntology.CONCEPT_TYPE_PRODUCTION_COMPANY, visibility);
        VisalloProperties.SOURCE.addPropertyValue(productionCompanyMutation, MULTI_VALUE_KEY, SOURCE, visibility);
        String name = json.optString("name");
        if (name != null && name.length() > 0) {
            VisalloProperties.TITLE.addPropertyValue(productionCompanyMutation, MULTI_VALUE_KEY, name, visibility);
        }
    ...
    private void mapPerson(int personId, JSONObject personJson, Context context) throws ParseException {
        ...
        VisalloProperties.CONCEPT_TYPE.setProperty(m, TheMovieDbOntology.CONCEPT_TYPE_PERSON, visibility);
        ...
        VisalloProperties.TITLE.addPropertyValue(m, MULTI_VALUE_KEY, name, visibility);

        String biography = personJson.optString("biography");
        if (biography != null) {
            Metadata metadata = new Metadata();
            VisalloProperties.TEXT_DESCRIPTION_METADATA.setMetadata(metadata, "Biography", defaultVisibility);
            VisalloProperties.MIME_TYPE_METADATA.setMetadata(metadata, "text/plain", defaultVisibility);
            StreamingPropertyValue value = new StreamingPropertyValue(new ByteArrayInputStream(biography.getBytes()), String.class);
            VisalloProperties.TEXT.addPropertyValue(m, MULTI_VALUE_KEY, value, metadata, visibility);
        }
    ...
    private void processPersonCredits(int personId, JSONObject personJson, Vertex personVertex) {
        ...
        for (int i = 0; i < cast.length(); i++) {
            ...
            VisalloProperties.CONCEPT_TYPE.setProperty(movieMutation, TheMovieDbOntology.CONCEPT_TYPE_MOVIE, visibility);
            VisalloProperties.SOURCE.addPropertyValue(movieMutation, MULTI_VALUE_KEY, SOURCE, visibility);
            String title = movieJson.optString("title");
            if (title != null && title.length() > 0) {
                VisalloProperties.TITLE.addPropertyValue(movieMutation, MULTI_VALUE_KEY, title, visibility);
            }
    ...
    private void mapMovie(int movieId, JSONObject movieJson, Context context) throws ParseException {
        ...
        VertexBuilder m = prepareVertex(vertexId, visibility);
        VisalloProperties.CONCEPT_TYPE.setProperty(m, TheMovieDbOntology.CONCEPT_TYPE_MOVIE, visibility);
        VisalloProperties.SOURCE.addPropertyValue(m, MULTI_VALUE_KEY, SOURCE, visibility);
        VisalloProperties.SOURCE_URL.addPropertyValue(m, MULTI_VALUE_KEY, sourceUrl, visibility);
        ...
        rawValue.searchIndex(false);
        VisalloProperties.RAW.setProperty(m, rawValue, visibility);
        VisalloProperties.TITLE.addPropertyValue(m, MULTI_VALUE_KEY, title, visibility);
        ...
        if (overview != null && overview.length() > 0) {
            Metadata metadata = new Metadata();
            VisalloProperties.TEXT_DESCRIPTION_METADATA.setMetadata(metadata, "Overview", defaultVisibility);
            VisalloProperties.MIME_TYPE_METADATA.setMetadata(metadata, "text/plain", defaultVisibility);
    ...
    private void processMovieProductionCompanies(int movieId, JSONObject movieJson, Vertex movieVertex) {
        ...
        if (productionCompanies != null) {
            for (int i = 0; i < productionCompanies.length(); i++) {
                ...
                VisalloProperties.CONCEPT_TYPE.setProperty(productionCompanyMutation, TheMovieDbOntology.CONCEPT_TYPE_PRODUCTION_COMPANY, visibility);
                VisalloProperties.SOURCE.addPropertyValue(productionCompanyMutation, MULTI_VALUE_KEY, SOURCE, visibility);
                VisalloProperties.SOURCE_URL.addPropertyValue(productionCompanyMutation, MULTI_VALUE_KEY, sourceUrl, visibility);
    ...
    private void processMovieCredits(int movieId, JSONObject movieJson, Vertex movieVertex) {
        ...
        for (int i = 0; i < cast.length(); i++) {
            ...
            VisalloProperties.CONCEPT_TYPE.setProperty(personMutation, TheMovieDbOntology.CONCEPT_TYPE_PERSON, visibility);
            VisalloProperties.SOURCE.addPropertyValue(personMutation, MULTI_VALUE_KEY, SOURCE, visibility);
            VisalloProperties.SOURCE_URL.addPropertyValue(personMutation, MULTI_VALUE_KEY, sourceUrl, visibility);
            String name = castJson.optString("name");
            if (name != null && name.length() > 0) {
                VisalloProperties.TITLE.addPropertyValue(personMutation, MULTI_VALUE_KEY, name, visibility);
            }
            ...
            if (character != null && character.length() > 0) {
                ...
                VisalloProperties.CONCEPT_TYPE.setProperty(roleMutation, TheMovieDbOntology.CONCEPT_TYPE_ROLE, visibility);
                VisalloProperties.SOURCE.addPropertyValue(roleMutation, MULTI_VALUE_KEY, SOURCE, visibility);
                VisalloProperties.TITLE.addPropertyValue(roleMutation, MULTI_VALUE_KEY, character, visibility);
