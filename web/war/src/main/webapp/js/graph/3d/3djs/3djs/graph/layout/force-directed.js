...
/*eslint camelcase:0, space-after-keywords:0*/
define([], function() {
    'use strict';
    ...
    var ForceDirected = function(graph, options) {
      ...
      this.init = function() {
        ...
        forceConstant = 100;
        attraction_constant = this.attractionMultiplier * forceConstant;
        repulsion_constant = this.repulsionMultiplier * forceConstant;
      };

      this.updateOptions = function(options) {
        if ('force' in options) {
            forceConstant = options.force || 100;
        }

        if ('attraction' in options) {
            this.attractionMultiplier = (options.attraction || 5);
            attraction_constant = this.attractionMultiplier * forceConstant;
        }

        if ('repulsion' in options) {
            this.repulsionMultiplier = (options.repulsion || 0.75)
            repulsion_constant = this.repulsionMultiplier * forceConstant;
        }
