...
define([
    ...
    GraphRenderer.prototype._init = function() {
        ...
        var windowResizeHandlerThrottled = _.throttle(windowResizeHandler, 250);
        window.addEventListener('resize', windowResizeHandlerThrottled, false);
        ...
        self.updateLayoutOptions = function(options) {
            if (self._layout && self._layout.updateOptions) {
                self._layout.updateOptions(options);
                self._layout.stopCalculating();
                setTimeout(function() {
        ...
        self.screenToWorld = function(screenX, screenY) {
            var x = (screenX / width) * 2 - 1,
    ...
    GraphRenderer.prototype.addToRenderLoop = function() {
        ...
        function render() {
            ...
            var eligible = false;
            if (self.mousedown) {
                if (!self.clickEligibleTimeout) {
                    self.clickEligible = true;
                    self.clickStart = _.clone(mouse);
                    self.clickEligibleTimeout = true;
                    setTimeout(function() {
                        self.clickEligible = false;
                    }, 250)
                }
            } else if (self.clickEligibleTimeout) {
                var dx = mouse.x - self.clickStart.x,
                    dy = mouse.y - self.clickStart.y,
                    d = Math.sqrt(dx * dx + dy * dy);
                if (d < 10) {
                    eligible = true;
                }
                self.clickEligibleTimeout = null;
                self.clickEligible = false;
            }
            ...
            if (!self.mousedown && eligible && self.checkPick) {
                pick();
    ...
    GraphRenderer.prototype._spriteForNode = function(node) {
        THREE.ImageUtils.crossOrigin = node.data.icon.match(new RegExp('^https?:\/\/' + location.host, 'i')) ?
            undefined : 'anonymous';
