...
import org.visallo.analystsNotebook.AnalystsNotebookExportConfiguration;
import org.visallo.analystsNotebook.AnalystsNotebookFeature;
...
public class ChartItem {
    ...
    public static ChartItem createFromVertexAndWorkspaceEntity(
        ...
        if (analystsNotebookExportConfiguration.includeImageUrl()) {
            String imageUrl = VisalloProperties.ENTITY_IMAGE_URL.getOnlyPropertyValue(vertex);
            if (imageUrl != null) {
        ...
        if (version.supports(AnalystsNotebookFeature.ICON_PICTURE) && analystsNotebookExportConfiguration.enableIconPicture()) {
            String mimeType = VisalloProperties.MIME_TYPE.getOnlyPropertyValue(vertex);
            if (mimeType != null && mimeType.toLowerCase().startsWith("image/")) {
                ...
                String imageVertexId = VisalloProperties.ENTITY_IMAGE_VERTEX_ID.getPropertyValue(vertex);
    ...
    private static byte[] getThumbnailBytes(Vertex vertex, ArtifactThumbnailRepository artifactThumbnailRepository, User user, AnalystsNotebookExportConfiguration analystsNotebookExportConfiguration) {
        ...
        if (thumbnail == null) {
            Property rawProperty = VisalloProperties.RAW.getProperty(vertex);
            ...
            try {
                thumbnail = artifactThumbnailRepository.createThumbnail(vertex, rawProperty.getKey(), type, in, dimensions, user);
            } catch (IOException e) {
                throw new VisalloException("error creating thumbnail", e);
            }
        }
