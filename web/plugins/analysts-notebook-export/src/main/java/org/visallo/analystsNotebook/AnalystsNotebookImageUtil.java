...
public class AnalystsNotebookImageUtil {
    private static final VisalloLogger LOGGER = VisalloLoggerFactory.getLogger(AnalystsNotebookImageUtil.class);
    ...
    public static byte[] convertImageFormat(byte[] data, String formatName, Integer intermediateBufferedImageType, Color intermediateBackgroundColor) {
        ...
        try {
            ...
            if (written) {
                ...
                throw new VisalloException("no appropriate writer found for format: " + formatName);
            }
        } catch (IOException e) {
            throw new VisalloException("IOException converting image format", e);
        }
