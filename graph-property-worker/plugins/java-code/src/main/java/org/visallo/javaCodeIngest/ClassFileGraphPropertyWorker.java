...
public class ClassFileGraphPropertyWorker extends GraphPropertyWorker {
    ...
    private VertexBuilder createClassVertexBuilder(String className, GraphPropertyWorkData data) {
        ...
        JavaCodeIngestOntology.CLASS_NAME.addPropertyValue(vertexBuilder, MULTI_VALUE_KEY, className, data.createPropertyMetadata(), data.getVisibility());
    ...
    private void createMethodVertex(Method method, Vertex classVertex, JavaClass javaClass, ConstantPoolGen constants, GraphPropertyWorkData data) {
        ...
        data.setVisibilityJsonOnElement(vertexBuilder);
        JavaCodeIngestOntology.METHOD_NAME.addPropertyValue(vertexBuilder, MULTI_VALUE_KEY, method.getName(), data.createPropertyMetadata(), data.getVisibility());
        JavaCodeIngestOntology.METHOD_SIGNATURE.addPropertyValue(vertexBuilder, MULTI_VALUE_KEY, method.getSignature(), data.createPropertyMetadata(), data.getVisibility());
        VisalloProperties.CONCEPT_TYPE.setProperty(vertexBuilder, JavaCodeIngestOntology.CONCEPT_TYPE_METHOD, data.createPropertyMetadata(), data.getVisibility());
        ...
        for (int i = 0; i < method.getArgumentTypes().length; i++) {
            ...
            data.setVisibilityJsonOnElement(edge, getAuthorizations());
            JavaCodeIngestOntology.ARGUMENT_NAME.addPropertyValue(edge, MULTI_VALUE_KEY, argumentName, data.createPropertyMetadata(), data.getVisibility(), getAuthorizations());
        ...
        for (InstructionHandle ih = mg.getInstructionList().getStart(); ih != null; ih = ih.getNext()) {
            ...
            if (i instanceof InvokeInstruction) {
                ...
                JavaCodeIngestOntology.METHOD_SIGNATURE.addPropertyValue(vertexBuilder, MULTI_VALUE_KEY, method.getSignature(), data.createPropertyMetadata(), data.getVisibility());
                VisalloProperties.CONCEPT_TYPE.setProperty(invokedMethodVertexBuilder, JavaCodeIngestOntology.CONCEPT_TYPE_METHOD, data.createPropertyMetadata(), data.getVisibility());
    ...
    private void createFieldVertex(Field field, Vertex classVertex, JavaClass javaClass, GraphPropertyWorkData data) {
        ...
        data.setVisibilityJsonOnElement(vertexBuilder);
        JavaCodeIngestOntology.FIELD_NAME.addPropertyValue(vertexBuilder, MULTI_VALUE_KEY, field.getName(), data.createPropertyMetadata(), data.getVisibility());
        VisalloProperties.CONCEPT_TYPE.setProperty(vertexBuilder, JavaCodeIngestOntology.CONCEPT_TYPE_FIELD, data.createPropertyMetadata(), data.getVisibility());
